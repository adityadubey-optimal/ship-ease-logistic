...
export default function GenericInput({
    ...
    // Base input style common to all variants
    const baseInputStyle: React.CSSProperties = {
        width: "100%",
        backgroundColor: "white",
        transition: "border-color 0.2s ease, box-shadow 0.2s ease",
    ...
    // Data table variant: more compact, matching table text sizing.
    const dataTableInputStyle: React.CSSProperties = {
        ...baseInputStyle,
        height: isMobile ? "1.75rem" : "2.5rem", // slightly smaller height
        padding: "0 1rem 0 0.75rem", // more compact padding
        fontSize: theme.fonts.web.body.size, // using body font size for table cells
        fontWeight: theme.fonts.web.body.weight,
    }

    // Choose style based on props
    const inputStyle = isForDataTable ? dataTableInputStyle : (isMobile ? mobileInputStyle : desktopInputStyle)
    ...
    return (
        <div style={containerStyle}>
            ...
            <div style={{ position: "relative" }}>
                <input id={id} value={value} onChange={handleChange} style={inputStyle} {...props} />
                {IconComponent && (
                    <button type="button" onClick={onIconClick} style={iconButtonStyle}>
                        <IconComponent style={iconStyle} />
                    </button>
                )}
            </div>
            {errorMessage && <p style={{ ...messageStyle, color: theme.colors.error }}>{errorMessage}</p>}
            {successMessage && <p style={{ ...messageStyle, color: theme.colors.success }}>{successMessage}</p>}
        </div>
    )
}